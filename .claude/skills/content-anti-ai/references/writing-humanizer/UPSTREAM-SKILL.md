...
鐵律：永遠不要用升華／訓誡收尾。永遠不要把整篇文章寫成大綱與條列。
```
...
| 「這篇本來就是分析文，用標題加條列比較清楚」 | 不行。論說文用段落推進。整篇大綱就是 AI 味。改回散文。 |
| 「結尾升華一下，讀者比較有共鳴」 | 不行。收在具體事實。升華＝AI。 |
| 「這句話對仗工整，當主題金句很適合」 | 不行。不要造口號，更不要在文中重複它。 |
| 「這幾個重點名詞加粗，閱讀比較有層次」 | 散文裡的關鍵詞粗體一律刪。靠句子結構強調。 |
...
- **結構與修辭模式（模式 25-31）** — 大綱骨架、四字標籤清單、意義蓋章、句內粗體、元論述、升華結尾、抽象事物具象成「一條線／路線」並反覆回指等中文論說文專屬破綻，見 [references/structure-rhetoric-patterns.md](references/structure-rhetoric-patterns.md)
...
- 粗體標題加冒號的列表？→ 改寫成散文
- 整篇是「標題＋編號＋條列」的大綱？→ 改回段落，最多留一兩處清單
- 清單項目是「**四字標籤**：解釋」？→ 拆掉標籤，寫成正常句子
- 每段都用「奠定基礎／具有重要意義」收尾？→ 刪除意義蓋章
- 散文句子裡有關鍵詞粗體？→ 刪除粗體
- 出現「本文將探討／接下來我們來看」？→ 刪除導讀宣告，直接講
- 結尾出現「願我們／歷史將會記住／神聖的」升華？→ 收在具體事實
- 有一句對仗金句被重複引用？→ 刪掉口號，不要重複
- 抽象事物被說成「一條線／路線」+ 同篇反覆回指 +「走出來／長出來」？→ 換回具體名詞或拆掉 motif（三信號疊加才算；單次的「這條路走不通」不要動）
- 引號是否統一使用「」和『』？→ 統一
